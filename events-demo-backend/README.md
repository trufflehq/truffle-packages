...
* Next, you should define the installation workflow inside the `installActionRel` in `truffle.config.mjs`. Inside the `installActionRel` you will need to update:
  ...
  * Update the `eventTopicPath` attributes to follow the path format for your package `@orgSlug/<packageSlug>@latest/_EventTopic/<eventTopicSlug from previous stem> e.g `@truffle/events-demo-backend@latest/_EventTopic/viewer-create-poll` -> `@truffle-dev-early-access/my-custom-event-package@latest/_EventTopic/custom-event-topic`
