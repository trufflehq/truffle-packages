...
const Giveaway = observer(
  ({ avatarUrl, creatorName, links }: GiveawayProps) => {
    useStyleSheet(styleSheet);
    useGoogleFontLoader(() => ["Inter"]);

    // TODO: don't render anything if extension isn't installed
    const extensionInfo$ = useSignal(jumper.call("context.getInfo"));

    if (!extensionInfo$.get()) {
      return <>Loading...</>;
    }

    const onSubmit = () => {
      // TODO: upsert FormResponse
      mutation();
    };
    ...
    return (
      <div className="c-giveaway">
        ...
        We just need your email to contact you if you win. We'll delete it after
        choosing a winner.
