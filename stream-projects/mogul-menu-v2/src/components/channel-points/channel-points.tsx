...
import { MOGUL_MENU_JUMPER_MESSAGES, useOrgUser$ } from "../../shared/mod.ts";
...
export default function ChannelPoints(
  ...
  const { channelPointsData, channelPointsError, reexecuteChannelPointsQuery } =
    useChannelPoints();

  const rawChannelPointsRef = useRef<number | undefined>();
  const rawChannelPoints = channelPointsData?.orgUserCounterType?.orgUserCounter
    ?.count;
  ...
  useEffect(() => {
    jumper.call("comms.onMessage", (message: string) => {
      if (message === GLOBAL_JUMPER_MESSAGES.INVALIDATE_USER) {
        refetchOrgUser({ requestPolicy: "network-only" });
      } else if (
        message === MOGUL_MENU_JUMPER_MESSAGES.INVALIDATE_CHANNEL_POINTS
      ) {
  ...
  const { canClaim$, claimCountdownMs$, claim, resetTimer } =
    useWatchtimeClaimCounter({
      sourceType: "youtube",
    });
  const canClaim = useSelector(() => canClaim$.get());
  const claimCountdownMs = useSelector(() => claimCountdownMs$.get());

  const title = canClaim ? "" : formatCountdown(claimCountdownMs / 1000, {
