...
export const createElement = function (element, props, ...children) {
  if (props?._truffleTreeSiblingIndex || props?._truffleTreePath) {
    ...
    return oldCreateElement(TruffleComponentWrapper, {
      childComponent: { element, props, children },
    });
  }
...
function TruffleComponentWrapper({ childComponent }) {
  ...
  const { truffleTreePath = props?._truffleTreePath } =
    useContext(TruffleTreePathContext) || {};
  ...
  if (foundComponentInstance?.props) {
    props = {
      ...props,
      ...castProps(foundComponentInstance.props, componentInstances),
    };
  }

  return (
    <TruffleTreePathContext.Provider
      value={{ truffleTreePath: childTruffleTreePath }}
    >
