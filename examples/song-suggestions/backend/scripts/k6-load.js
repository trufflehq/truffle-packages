...
// TO RUN `k6 run --vus 10 --duration 30s k6-load.js`
...
function makeRequest(url, orgId) {
  ...
  const params = {
    headers: {
      ...
      "Authorization": `Bearer ${__ENV.TRUFFLE_API_KEY}`,
      "x-org-id": orgId
    },
  };
  ...
  check(res, {
    'verify collectible redeemed successfully': (r) => {
      console.log(JSON.stringify(r.body))
      const response = JSON.parse(r.body)
      console.log('response', JSON.stringify(response.data))
      return response.data.ownedCollectibleRedeem.redeemResponse.hasBroadcastEvent === true
    }
  })
}
export default function () {
  makeRequest('https://mycelium.truffle.vip/graphql', "8e35b570-6c2f-11ec-bade-b32a8d305590")
}