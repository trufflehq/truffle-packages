...
import { useGoogleFontLoader, useFetchPoll, useInterval } from "../../hooks/mod.ts";
import { getClearSubmissionInput, fetchSubmissionCount } from "../../api/mod.ts"
import { ACTION_EXECUTE_MUTATION } from "../../gql/mod.ts"
import { Submission, Poll } from '../../types/mod.ts'
...
export default function Admin() {
  ...
  const hasAdminPerms = hasPermission(activeOrgUser, ["admin", "moderator", 'mod']);
  ...
  return (
    <div className="c-admin">
      ...
      <div className="auth">
        <UserInfo setActiveUser={setActiveUser} setActiveOrgUser={setActiveOrgUser} />
      </div>
      {hasAdminPerms ? <AdminDashboard /> : activeOrgUser?.roleConnection && activeUser?.name
        ? (
          <div className="perms">
...
export interface RandomSubmissionProps {
  ...
  setRandomSubmission: React.Dispatch<React.SetStateAction<Submission | null | undefined>>  
}
...
function AdminDashboard() {
  ...
  return (
    <main className="c-admin-dashboard">
      <PollManager latestPoll={latestPoll} randomSubmission={randomSubmission} setRandomSubmission={setRandomSubmission} />
      <AdminDashboardTabs latestPoll={latestPoll} randomSubmission={randomSubmission} setRandomSubmission={setRandomSubmission} />
    </main>
  );
}
...
function ClearButton({ onClearFn }: { onClearFn: () => void}) {
  ...
  const onClear = async () => {
    if(confirm("This will clear all submissions, including the approved ones")) {
      setIsClearing(true)
      console.log('clear')
...
function Controls() {
  ...
  const refreshCount = async () => {
   const submissionCount = await fetchSubmissionCount()
    // if(typeof submissionCount !== 'undefined' && typeof submissionCount === 'number') {
  ...
  return <div className="c-controls">
        ...
        {submissionCount ? <div className='count'>
    ...
    <div className="tabs">
      <Tabs defaultValue="approved">
        ...
        <TabPanel value="submissions">
          <SubmissionsList variant={"submissions"} {...props} />
        </TabPanel>
